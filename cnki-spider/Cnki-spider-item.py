...
def output():
    df = pd.DataFrame(papers,
                      columns=['关键字1', '关键词2', '关键字3', '正文标题', '副标题', '正文快照', '报纸日期', '版名', '版号', '分类号', '报纸名',
                               '报纸所在地',
...
if __name__ == '__main__':
    ...
    with open('result_urls_20171120.csv', 'r', encoding='utf-8') as fr:
        lines = fr.readlines()
        for one in lines:
            items = one.split(',')
            # count = items[0]
            ...
            try:
                driver.get(items[1])
                parse_one_pager(driver.page_source)
            except Exception as e:
                print(e)
                print("第 {} 篇文章解析出错。".format(items[0]))
                continue
        output()
